...
/// One specialization-safe subresultant for an eliminated-parameter fiber.
#[derive(Clone, Debug, PartialEq)]
pub struct CurveIntersectionFiberSubresultant {
    /// Subresultant index and maximum degree in the fiber parameter.
    pub index: usize,
    /// Conservative retained-parameter interpolation degree.
    pub degree_bound: usize,
    /// Exact polynomial in the original `(first, second)` parameter order.
    pub polynomial: BivariatePolynomial,
}

/// Exact subresultant sequence for recovering a higher-nullity parameter fiber.
#[derive(Clone, Debug, PartialEq)]
pub struct CurveIntersectionFiberSubresultantReport {
    /// Final construction status.
    pub status: CurveIntersectionParameterLiftStatus,
    /// Parameter retained in every coefficient polynomial.
    pub retained_parameter: CurveResultantParameter,
    /// Parameter whose specialized common factor is recovered.
    pub fiber_parameter: CurveResultantParameter,
    /// Positive-index subresultants in increasing index order.
    pub subresultants: Vec<CurveIntersectionFiberSubresultant>,
    /// Exact determinant failure, when construction stopped at that boundary.
    pub determinant_error: Option<BareissError>,
}
...
/// Constructs specialization-safe subresultants for higher-nullity fibers.
///
/// For a fixed retained parameter, the first subresultant with a nonzero
/// principal coefficient is, up to a nonzero scalar, the GCD of the two
/// specialized equations in the fiber parameter. Every coefficient is built
/// as a Sylvester minor at generic exact samples and interpolated in the
/// retained parameter. Unlike a pseudo-remainder chain, these determinantal
/// polynomials specialize correctly when the common-root multiplicity grows.
/// The terminal entry is the lower-degree input equation; it covers a fiber
/// where all preceding principal subresultants vanish.
pub fn fiber_subresultants_bivariate_polynomial_system(
    first_equation: &BivariatePolynomial,
    second_equation: &BivariatePolynomial,
    retained_parameter: CurveResultantParameter,
    config: CurveIntersectionResultantConfig,
) -> CurveIntersectionFiberSubresultantReport {
    let fiber_parameter = opposite_parameter(retained_parameter);
    if bivariate_polynomial_is_empty(first_equation)
        || bivariate_polynomial_is_empty(second_equation)
    {
        return fiber_subresultant_report(
            CurveIntersectionParameterLiftStatus::EmptyEquation,
            retained_parameter,
            fiber_parameter,
            Vec::new(),
    ...
    let (first_retained_degree, first_fiber_degree) =
        oriented_bidegree(first_degree, retained_parameter);
    let (second_retained_degree, second_fiber_degree) =
        oriented_bidegree(second_degree, retained_parameter);
    let terminal_index = first_fiber_degree.min(second_fiber_degree);
    if terminal_index == 0 {
        return fiber_subresultant_report(
            CurveIntersectionParameterLiftStatus::UnsupportedEliminatedDegree,
            retained_parameter,
    ...
    let mut subresultants = Vec::with_capacity(terminal_index);
    for index in 1..terminal_index {
        let degree_bound = (second_fiber_degree - index) * first_retained_degree
            + (first_fiber_degree - index) * second_retained_degree;
        if degree_bound > config.max_resultant_degree {
            return fiber_subresultant_report(
                CurveIntersectionParameterLiftStatus::DegreeBoundExceeded,
                retained_parameter,
        ...
        let mut parameters = Vec::with_capacity(degree_bound + 1);
        let mut coefficient_samples = vec![Vec::with_capacity(degree_bound + 1); index + 1];
        let mut sample_index = 0_usize;
        while parameters.len() <= degree_bound {
            let parameter = Real::from(sample_index as u64);
            sample_index += 1;
            ...
            if first_sample_degree != Some(first_fiber_degree)
                || second_sample_degree != Some(second_fiber_degree)
            {
                continue;
            }
            let coefficients =
                match subresultant_coefficients(&first, &second, index, config.min_precision) {
        ...
        let mut coefficient_polynomials = Vec::with_capacity(index + 1);
        for samples in coefficient_samples {
            let Some(coefficients) =
                interpolate_parameter_values(&parameters, &samples, config.min_precision)
            else {
                return fiber_subresultant_report(
                    CurveIntersectionParameterLiftStatus::InterpolationFailed,
                    retained_parameter,
            ...
            coefficient_polynomials.push(coefficients);
        }
        subresultants.push(CurveIntersectionFiberSubresultant {
            index,
            degree_bound,
            polynomial: orient_fiber_coefficient_polynomials(
    ...
    let (terminal, degree_bound) = if first_fiber_degree <= second_fiber_degree {
        (first_equation.clone(), first_retained_degree)
    } else {
        (second_equation.clone(), second_retained_degree)
    };
    subresultants.push(CurveIntersectionFiberSubresultant {
...
fn fiber_subresultant_report(
    status: CurveIntersectionParameterLiftStatus,
    retained_parameter: CurveResultantParameter,
    fiber_parameter: CurveResultantParameter,
    subresultants: Vec<CurveIntersectionFiberSubresultant>,
    determinant_error: Option<BareissError>,
) -> CurveIntersectionFiberSubresultantReport {
    CurveIntersectionFiberSubresultantReport {
        status,
...
fn orient_fiber_coefficient_polynomials(
    coefficient_polynomials: Vec<Vec<Real>>,
    retained_parameter: CurveResultantParameter,
) -> BivariatePolynomial {
    let retained_count = coefficient_polynomials
        .iter()
        .map(Vec::len)
        .max()
        .unwrap_or(0);
    match retained_parameter {
        CurveResultantParameter::First => {
            let mut coefficients =
                vec![vec![Real::zero(); coefficient_polynomials.len()]; retained_count];
            for (fiber_power, polynomial) in coefficient_polynomials.iter().enumerate() {
                for (retained_power, coefficient) in polynomial.iter().enumerate() {
                    coefficients[retained_power][fiber_power] = coefficient.clone();
                }
            }
            BivariatePolynomial::new(coefficients)
        }
        CurveResultantParameter::Second => {
            let mut coefficients = Vec::with_capacity(coefficient_polynomials.len());
            for polynomial in coefficient_polynomials {
                let mut row = polynomial;
                row.resize(retained_count, Real::zero());
...
fn subresultant_coefficients(
    ...
    debug_assert!(index > 0 && index < first_degree.min(second_degree));
    let row_count = first_degree + second_degree - 2 * index;
    let column_count = first_degree + second_degree - index;
    let first_row_count = second_degree - index;
    let second_row_count = first_degree - index;
    ...
    let mut coefficients = Vec::with_capacity(index + 1);
    for power in 0..=index {
        ...
        let minor = matrix
            ...
            .collect::<Vec<_>>();
        coefficients.push(determinant_bareiss(&minor, min_precision)?.determinant);
    }
    Ok(coefficients)
}
...
mod tests {
    ...
    #[test]
    fn determinantal_subresultants_recover_a_degree_two_common_factor() {
        // `(u^2-1)(u+2)` and `(u^2-1)(u^2+1)` have GCD `u^2-1`.
        let first = vec![real(-2), real(-1), real(2), real(1)];
        let second = vec![real(-1), real(0), real(0), real(0), real(1)];
        let first_subresultant = subresultant_coefficients(&first, &second, 1, -64).unwrap();
        assert_eq!(first_subresultant, vec![real(0), real(0)]);

        let second_subresultant = subresultant_coefficients(&first, &second, 2, -64).unwrap();
        assert_ne!(second_subresultant[2], real(0));
        assert_eq!(second_subresultant[1], real(0));
        assert_eq!(second_subresultant[0], -second_subresultant[2].clone());
    }

    #[test]
    fn bivariate_fiber_subresultants_specialize_in_either_orientation() {
        // At retained `t=0`, the equations specialize to the pair above. Away
        // from zero, the `+t` term removes the common factor.
        let first = BivariatePolynomial::new(vec![vec![real(-2), real(-1), real(2), real(1)]]);
        let second = BivariatePolynomial::new(vec![
            vec![real(-1), real(0), real(0), real(0), real(1)],
            vec![real(1)],
        ]);
        let transpose = |polynomial: &BivariatePolynomial| {
        ...
        for (first, second, retained_parameter) in [
            ...
            let report = fiber_subresultants_bivariate_polynomial_system(
                &first,
                &second,
                retained_parameter,
                CurveIntersectionResultantConfig::default(),
            ...
            assert_eq!(report.subresultants.len(), 3);
            assert_eq!(report.subresultants[0].index, 1);
            assert_eq!(report.subresultants[1].index, 2);
            assert_eq!(report.subresultants[2].index, 3);

            let first_specialized = evaluate_bivariate_at_retained_parameter(
                &report.subresultants[0].polynomial,
                &real(0),
                retained_parameter,
            );
            assert!(first_specialized.iter().all(|value| value == &real(0)));
            let second_specialized = evaluate_bivariate_at_retained_parameter(
                &report.subresultants[1].polynomial,
                &real(0),
                retained_parameter,
            );
            assert_ne!(second_specialized[2], real(0));
            assert_eq!(second_specialized[1], real(0));
            assert_eq!(second_specialized[0], -second_specialized[2].clone());
        }
    }
