...
/// Retained 3D cubic Bezier carrier.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SketchCubic3 {
    /// First control point.
    pub p0: SketchEntityHandle,
    /// Second control point.
    pub p1: SketchEntityHandle,
...
pub enum SketchEntityKind {
    ...
    Cubic2(SketchCubic2),
    /// 3D cubic Bezier carrier retained for projected workplane replay.
    Cubic3(SketchCubic3),
...
pub enum SketchConstraintKind {
    ...
    /// Projected 3D point-pair distance equal to a retained circle/arc radius.
    ///
    /// Lowering projects the retained 3D point-pair displacement into a
    ...
    ProjectedPointRadiusEquality3 {
        /// Workplane entity that supplies origin and normal/quaternion.
        workplane: SketchEntityHandle,
        /// First 3D point of the projected distance.
        a: SketchEntityHandle,
        /// Second 3D point of the projected distance.
        b: SketchEntityHandle,
    ...
    /// and replays `|a-b|_uv^2 - |c-d|_uv^2 == 0` plus the unit-quaternion
    ...
    ProjectedEqualPointPointDistances3 {
        /// Workplane entity that supplies origin and normal/quaternion.
        workplane: SketchEntityHandle,
        /// First point of the first projected distance.
        a: SketchEntityHandle,
        /// Second point of the first projected distance.
        b: SketchEntityHandle,
        /// First point of the second projected distance.
        c: SketchEntityHandle,
        /// Second point of the second projected distance.
    ...
    /// Equality between a point-pair distance and a point-line distance after projection.
    ///
    /// Lowering projects the retained 3D point pair and the retained 3D
    /// point/line pair into the same workplane frame, then clears the
    /// projected line denominator:
    /// `|a-b|_uv^2 * |line_uv|^2 - cross(point-line_start, line_uv)^2 == 0`.
    ...
    ProjectedEqualPointDistancePointLineDistance3 {
        /// Workplane entity that supplies origin and normal/quaternion.
        workplane: SketchEntityHandle,
        /// First 3D point of the point-pair distance.
        a: SketchEntityHandle,
        /// Second 3D point of the point-pair distance.
        b: SketchEntityHandle,
    ...
    /// Projected 3D point-pair distance-difference relation in a retained workplane.
    ///
    /// The retained relation is `distance(longer_a,longer_b)_uv =
    /// distance(shorter_a,shorter_b)_uv + difference`. Lowering reuses the
    /// square-root-free proof package `(L + S - d^2)^2 - 4LS == 0` plus
    ...
    ProjectedPointDistanceDifference3 {
        /// Workplane entity that supplies origin and normal/quaternion.
        workplane: SketchEntityHandle,
        /// First endpoint of the longer projected point-pair distance.
        longer_a: SketchEntityHandle,
        /// Second endpoint of the longer projected point-pair distance.
        longer_b: SketchEntityHandle,
        /// First endpoint of the shorter projected point-pair distance.
        shorter_a: SketchEntityHandle,
        /// Second endpoint of the shorter projected point-pair distance.
        shorter_b: SketchEntityHandle,
        /// Distance entity for the exact projected distance difference.
        difference: SketchEntityHandle,
    },
    ...
    /// 2D circular-arc endpoint tangent to a projected 3D cubic Bezier.
    ///
    /// Lowering proves the workplane unit-quaternion guard, projects the 3D
    /// cubic control net relative to the retained workplane origin, evaluates
    /// the projected cubic point and derivative exactly in Bernstein form, and
    ...
    ProjectedArcCubicCurveTangent3 {
        ...
        /// Curve parameter used for projected point and derivative evaluation.
    ...
    /// Projected 3D line endpoint tangent to a retained 2D cubic Bezier.
    ///
    /// Lowering proves the workplane unit-quaternion guard, projects the
    /// selected 3D line endpoint and outgoing 3D line tangent into exact
    /// workplane `U/V` coordinates, evaluates the retained cubic point and
    /// derivative exactly in Bernstein form, then emits endpoint incidence,
    ...
    /// Projected G1 tangency between two retained 3D cubic Bezier carriers.
    ///
    /// Lowering proves the workplane unit-quaternion guard, projects both 3D
    /// cubic control nets relative to the same workplane origin, evaluates
    /// both projected cubic points and derivatives exactly in Bernstein form,
    /// then replays point incidence, derivative support, and same-direction
    /// branch rows. Projection is linear, so the derivative of each projected
    ...
    /// Projected parametric C2 continuity between two retained 3D cubic Beziers.
    ///
    /// Lowering proves the workplane unit-quaternion guard, projects both 3D
    /// cubic control nets relative to the same retained workplane origin, and
    /// evaluates point, first derivative, and second derivative exactly in
    /// Bernstein form at the retained parameters. It then emits coordinate
    /// equality rows for `B_a(t_a) = B_b(t_b)`,
    /// `B_a'(t_a) = B_b'(t_b)`, and `B_a''(t_a) = B_b''(t_b)` in projected
    ...
    /// 3D/workplane point-on-3D-cubic-Bezier incidence at a retained parameter.
    ///
    /// Lowering first proves the workplane unit-quaternion guard, projects the
    /// retained 3D point and all four retained 3D cubic control points relative
    /// to the workplane origin, evaluates the projected cubic in exact
...
pub enum SketchResidualStrategy {
    ...
    /// Squared equality between two point-pair distances after exact workplane projection.
    ...
    SquaredProjectedPointDistanceDifference,
    ...
    /// Squared equality between projected point-pair distance and point-line distance.
    SquaredProjectedPointDistancePointLineDistance,
    ...
    /// Exact workplane-projected 3D cubic-Bezier coordinate incidence package.
    ProjectedCubicCurveIncidence,
    ...
    /// Exact workplane-projected 3D cubic/3D cubic G1 tangency package.
    ProjectedCubicCurveCubicCurveTangent,
...
pub enum SketchResidualFormKind {
    ...
    /// Squared equality between two projected point-pair distances.
    ...
    /// Squared projected point-pair distance difference polynomial proof row.
    SquaredProjectedPointDistanceDifferencePolynomial,
    /// Projected point-pair distance difference branch predicate.
    ProjectedPointDistanceDifferenceBranchPredicate,
    ...
    /// Squared projected point-pair distance to point-line distance proof row.
    SquaredProjectedPointDistancePointLineDistancePolynomial,
    ...
    /// Exact selected endpoint-on-radius row for projected 3D cubic/arc tangency.
    ProjectedArcCubicCurveTangentEndpointRadiusPolynomial,
    /// Exact projected cubic point/arc endpoint incidence row.
    ProjectedArcCubicCurveTangentEndpointIncidencePolynomial,
    /// Exact projected radius/cubic derivative perpendicularity row.
    ...
    /// Exact projected endpoint coordinate-incidence row for cubic-line tangency.
    ProjectedCubicLineTangentEndpointIncidencePolynomial,
    /// Exact projected cubic derivative/line tangent cross-product proof row.
    ...
    /// Exact projected 3D cubic/cubic point incidence proof row.
    ProjectedCubicCurveCubicCurveTangentPointIncidencePolynomial,
    /// Exact projected 3D cubic/cubic derivative cross-product proof row.
    ProjectedCubicCurveCubicCurveTangentCrossProductPredicate,
    /// Exact projected 3D cubic/cubic derivative dot-product branch predicate.
    ProjectedCubicCurveCubicCurveTangentDotProductPredicate,
    /// Exact projected 3D cubic/cubic C2 point coordinate equality proof row.
    ProjectedCubicCurveCubicCurveC2PointPolynomial,
    /// Exact projected 3D cubic/cubic C2 first-derivative equality proof row.
    ...
    /// Exact projected 3D cubic-Bezier coordinate incidence proof row.
    ProjectedCubicCurveIncidencePolynomial,
}
...
impl SketchSolveProblem {
    ...
    /// Add a retained 3D cubic Bezier carrier.
    pub fn add_cubic3(
        &mut self,
        name: impl Into<String>,
        p0: SketchEntityHandle,
        ...
        self.add_entity(
            name,
            SketchEntityKind::Cubic3(SketchCubic3 { p0, p1, p2, p3 }),
        )
    }
    ...
    /// Add projected point-pair distance to circle/arc radius equality.
    pub fn add_projected_point_radius_equality3(
        &mut self,
        name: impl Into<String>,
        workplane: SketchEntityHandle,
        a: SketchEntityHandle,
    ...
    ///
    /// Lowering emits the unit-workplane guard and a single exact polynomial
    /// row comparing the two squared projected point-pair distances.
    pub fn add_projected_equal_point_point_distances3(
        &mut self,
        name: impl Into<String>,
        workplane: SketchEntityHandle,
        a: SketchEntityHandle,
        ...
        distance::projected_equal_point_point_distances3(self, name, workplane, a, b, c, d).handle
    }
    ...
    /// Add a retained 3D/workplane arc/cubic tangent relation.
    ///
    /// The projected 3D cubic point must coincide with the selected arc
    /// endpoint, and the projected cubic derivative replays exact
    /// radius-perpendicularity plus the signed tangent branch.
    #[allow(clippy::too_many_arguments)]
    pub fn add_projected_arc_cubic_curve_tangent3(
        &mut self,
        name: impl Into<String>,
        workplane: SketchEntityHandle,
        arc: SketchEntityHandle,
    ...
    /// Add a workplane-projected distance-difference constraint for two point pairs.
    ///
    /// Lowering emits the unit-workplane guard, a square-root-free polynomial
    /// equality, and the exact branch inequality selecting the longer
    /// point-pair distance.
    pub fn add_projected_point_distance_difference3(
        &mut self,
        name: impl Into<String>,
        workplane: SketchEntityHandle,
        ...
        distance::projected_point_distance_difference3(
            self, name, workplane, longer, shorter, difference,
        )
        .handle
    }
    ...
    pub fn add_projected_equal_point_distance_point_line_distance3(
        &mut self,
        name: impl Into<String>,
        workplane: SketchEntityHandle,
        a: SketchEntityHandle,
        ...
        distance::projected_equal_point_distance_point_line_distance3(
            self, name, workplane, a, b, point, line,
        )
        .handle
    }
    ...
    /// Add projected 3D point-on-3D-cubic incidence at a retained parameter.
    ///
    /// The point and cubic control net are projected into the workplane before
    /// exact Bernstein-coordinate replay. Segment-domain policy for the
    /// parameter remains explicit.
    pub fn add_projected_point_on_cubic_curve3(
        &mut self,
        name: impl Into<String>,
        workplane: SketchEntityHandle,
        point: SketchEntityHandle,
        ...
        incidence::projected_point_on_cubic_curve3(self, name, workplane, point, cubic, parameter)
            .handle
    }

    /// Add projected 3D line/cubic tangency at a retained cubic parameter.
    ///
    /// The selected 3D line endpoint and outgoing tangent are projected into
    /// the workplane and replayed against the exact 2D cubic point and
    /// derivative. The same-direction tangent branch is explicit.
    pub fn add_projected_cubic_line_tangent3(
        &mut self,
        name: impl Into<String>,
        workplane: SketchEntityHandle,
        cubic: SketchEntityHandle,
    ...
    ///
    /// Both cubic control nets are projected into the retained workplane, then
    /// exact Bernstein point/derivative replay emits point incidence,
    /// derivative collinearity, and same-direction branch rows.
    pub fn add_projected_cubic_curve_cubic_curve_tangent3(
        &mut self,
        name: impl Into<String>,
        workplane: SketchEntityHandle,
        first: SketchEntityHandle,
        ...
        tangency::projected_cubic_curve_cubic_curve_tangent3(
            self,
            name,
            workplane,
            first,
    ...
    /// then exact Bernstein replay emits projected point, velocity, and
    /// acceleration equality rows.
    pub fn add_projected_cubic_curve_cubic_curve_c2_continuity3(
        &mut self,
        name: impl Into<String>,
        workplane: SketchEntityHandle,
        first: SketchEntityHandle,
    ...
    pub fn residual_forms_for_constraint(
        ...
        match constraint.kind {
            ...
            SketchConstraintKind::ProjectedPointRadiusEquality3 {
                workplane,
                a,
                b,
                curve,
                ...
                let mut diagnostics = Vec::new();
                let Some(parts) = self.projected_point_radius_equality_exprs(
                    workplane,
                    a,
                    b,
                    curve,
            ...
            SketchConstraintKind::ProjectedEqualPointPointDistances3 {
                workplane,
                a,
                b,
                c,
                ...
                SketchResidualFormsReport {
                    ...
                    forms: vec![
                        ...
                        SketchResidualForm {
                            kind: SketchResidualFormKind::SquaredProjectedEqualPointPointDistancesPolynomial,
                            role: SketchResidualFormRole::ExactProof,
                            strategy: Some(
            ...
            SketchConstraintKind::ProjectedPointDistanceDifference3 {
                workplane,
                longer_a,
                longer_b,
                shorter_a,
                ...
                let mut diagnostics = Vec::new();
                let Some((parts, difference)) = self.projected_point_distance_difference_exprs(
                    workplane,
                    (longer_a, longer_b),
                    (shorter_a, shorter_b),
                    difference,
                ...
                SketchResidualFormsReport {
                    ...
                    forms: vec![
                        ...
                        SketchResidualForm {
                            kind: SketchResidualFormKind::SquaredProjectedPointDistanceDifferencePolynomial,
                            role: SketchResidualFormRole::ExactProof,
                            strategy: Some(
                                SketchResidualStrategy::SquaredProjectedPointDistanceDifference,
                            ),
                            residual: sum_minus_difference.clone() * sum_minus_difference
                                - Expr::int(4)
                                    * parts.first_squared.clone()
                        ...
                        SketchResidualForm {
                            kind: SketchResidualFormKind::ProjectedPointDistanceDifferenceBranchPredicate,
                            role: SketchResidualFormRole::ExactProof,
                            strategy: Some(
                                SketchResidualStrategy::SquaredProjectedPointDistanceDifference,
                            ),
            ...
            SketchConstraintKind::ProjectedEqualPointDistancePointLineDistance3 {
                workplane,
                a,
                b,
                point,
                ...
                SketchResidualFormsReport {
                    ...
                    forms: vec![
                        ...
                        SketchResidualForm {
                            kind: SketchResidualFormKind::SquaredProjectedPointDistancePointLineDistancePolynomial,
                            role: SketchResidualFormRole::ExactProof,
                            strategy: Some(
                                SketchResidualStrategy::SquaredProjectedPointDistancePointLineDistance,
                            ),
                            residual: parts.point_squared * parts.distance_denominator
                                - parts.distance_numerator,
                        },
                    ],
                    diagnostics,
            ...
            SketchConstraintKind::ProjectedPointOnCubicCurve3 {
                workplane,
                point,
                cubic,
                parameter,
                ...
                let mut diagnostics = Vec::new();
                let Some(parts) = self.projected_point_on_cubic_curve3_exprs(
                    workplane,
                    point,
                    cubic,
                    parameter,
                ...
                forms.extend(
                    parts
                        ...
                        .into_iter()
                        .map(|residual| SketchResidualForm {
                            kind: SketchResidualFormKind::ProjectedCubicCurveIncidencePolynomial,
                            role: SketchResidualFormRole::ExactProof,
                            strategy: Some(SketchResidualStrategy::ProjectedCubicCurveIncidence),
                            residual,
                        }),
                );
                SketchResidualFormsReport {
            ...
            SketchConstraintKind::ProjectedCubicLineTangent3 {
                workplane,
                cubic,
                parameter,
                line,
                ...
                let mut diagnostics = Vec::new();
                let Some(parts) = self.projected_cubic_line_tangent_exprs(
                    workplane,
                    cubic,
                    parameter,
                    line,
                ...
                forms.extend(parts.tangent.endpoint_incidence.into_iter().map(|residual| {
                    SketchResidualForm {
                        kind: SketchResidualFormKind::ProjectedCubicLineTangentEndpointIncidencePolynomial,
                        role: SketchResidualFormRole::ExactProof,
                        strategy: Some(SketchResidualStrategy::ProjectedCubicLineTangent),
                        residual,
                    }
                }));
                forms.push(SketchResidualForm {
                    kind: SketchResidualFormKind::ProjectedCubicLineTangentCrossProductPredicate,
                    role: SketchResidualFormRole::ExactProof,
                    strategy: Some(SketchResidualStrategy::ProjectedCubicLineTangent),
                    residual: parts.tangent.tangent_cross,
                });
                forms.push(SketchResidualForm {
                    kind: SketchResidualFormKind::ProjectedCubicLineTangentDotProductPredicate,
                    role: SketchResidualFormRole::ExactProof,
                    strategy: Some(SketchResidualStrategy::ProjectedCubicLineTangent),
            ...
            SketchConstraintKind::ProjectedCubicCurveCubicCurveTangent3 {
                workplane,
                first,
                first_parameter,
                second,
                ...
                let mut diagnostics = Vec::new();
                let Some(parts) = self.projected_cubic_curve_cubic_curve_tangent_exprs(
                    workplane,
                    first,
                    first_parameter,
                    second,
                ...
                forms.extend(parts.tangent.point_incidence.into_iter().map(|residual| {
                    SketchResidualForm {
                        kind: SketchResidualFormKind::ProjectedCubicCurveCubicCurveTangentPointIncidencePolynomial,
                        role: SketchResidualFormRole::ExactProof,
                        strategy: Some(SketchResidualStrategy::ProjectedCubicCurveCubicCurveTangent),
                        residual,
                    }
                }));
                forms.push(SketchResidualForm {
                    kind: SketchResidualFormKind::ProjectedCubicCurveCubicCurveTangentCrossProductPredicate,
                    role: SketchResidualFormRole::ExactProof,
                    strategy: Some(SketchResidualStrategy::ProjectedCubicCurveCubicCurveTangent),
                    residual: parts.tangent.tangent_cross,
                });
                forms.push(SketchResidualForm {
                    kind: SketchResidualFormKind::ProjectedCubicCurveCubicCurveTangentDotProductPredicate,
                    role: SketchResidualFormRole::ExactProof,
                    strategy: Some(SketchResidualStrategy::ProjectedCubicCurveCubicCurveTangent),
                    residual: parts.tangent.tangent_dot,
                });
            ...
            SketchConstraintKind::ProjectedCubicCurveCubicCurveC2Continuity3 {
                workplane,
                first,
                first_parameter,
                second,
                ...
                let mut diagnostics = Vec::new();
                let Some(parts) = self.projected_cubic_curve_cubic_curve_c2_exprs(
                    workplane,
                    first,
                    first_parameter,
                    second,
                ...
                forms.extend(
                    parts
                        ...
                        .map(|residual| {
                            SketchResidualForm {
                    kind: SketchResidualFormKind::ProjectedCubicCurveCubicCurveC2PointPolynomial,
                    role: SketchResidualFormRole::ExactProof,
                    strategy: Some(
                ...
                forms.extend(parts.c2.first_derivative.into_iter().map(|residual| {
                    SketchResidualForm {
                        kind: SketchResidualFormKind::ProjectedCubicCurveCubicCurveC2FirstDerivativePolynomial,
                        role: SketchResidualFormRole::ExactProof,
                        strategy: Some(
                            SketchResidualStrategy::ProjectedCubicCurveCubicCurveC2Continuity,
                        ),
                ...
                forms.extend(parts.c2.second_derivative.into_iter().map(|residual| {
                    SketchResidualForm {
                        kind: SketchResidualFormKind::ProjectedCubicCurveCubicCurveC2SecondDerivativePolynomial,
                        role: SketchResidualFormRole::ExactProof,
                        strategy: Some(
                            SketchResidualStrategy::ProjectedCubicCurveCubicCurveC2Continuity,
                        ),
            ...
            SketchConstraintKind::ProjectedArcCubicCurveTangent3 {
                workplane,
                arc,
                arc_endpoint,
                cubic,
                ...
                let mut diagnostics = Vec::new();
                let Some(parts) = self.projected_arc_cubic_curve_tangent_exprs(
                    workplane,
                    arc,
                    arc_endpoint,
                    cubic,
                ...
                let mut forms = vec![
                    ...
                    SketchResidualForm {
                        kind: SketchResidualFormKind::ProjectedArcCubicCurveTangentEndpointRadiusPolynomial,
                        role: SketchResidualFormRole::ExactProof,
                        strategy: Some(SketchResidualStrategy::ProjectedArcCubicCurveTangent),
                        residual: parts.tangent.arc_endpoint_radius,
                    },
                    SketchResidualForm {
                        kind: SketchResidualFormKind::ProjectedArcCubicCurveTangentEndpointIncidencePolynomial,
                        role: SketchResidualFormRole::ExactProof,
                        strategy: Some(SketchResidualStrategy::ProjectedArcCubicCurveTangent),
                        residual: parts.tangent.endpoint_incidence[0].clone(),
                    },
                    ...
                    SketchResidualForm {
                        kind: SketchResidualFormKind::ProjectedArcCubicCurveTangentRadiusPerpendicularPolynomial,
                        role: SketchResidualFormRole::ExactProof,
                        strategy: Some(SketchResidualStrategy::ProjectedArcCubicCurveTangent),
                        residual: parts.tangent.radius_perpendicular,
                    },
                ...
                forms.push(SketchResidualForm {
                    kind: SketchResidualFormKind::ProjectedArcCubicCurveTangentOrientationPredicate,
                    role: SketchResidualFormRole::ExactProof,
                    strategy: Some(SketchResidualStrategy::ProjectedArcCubicCurveTangent),
                    residual: parts.tangent.orientation,
                });
    ...
    fn lower_constraint(
        ...
        match constraint.kind {
            ...
            SketchConstraintKind::ProjectedPointRadiusEquality3 {
                ...
                // Compare a retained projected point-pair distance directly
                // to a circle/arc radius. Keeping this separate from helper
                // line construction preserves the source relation in exact
                // replay and failed-row diagnostics.
                let Some(parts) = self.projected_point_radius_equality_exprs(
                    workplane, a, b, curve, constraint, rows,
                ) else {
                    return;
                };
                self.push_residual(
                ...
                self.push_residual(
                    ...
                    rows,
                    constraint,
                    format!("{} projected point radius equality", constraint.name),
                    parts.point_squared - parts.radius.clone() * parts.radius,
                    SketchResidualStrategy::SquaredProjectedPointRadiusEquality,
                );
            }
            SketchConstraintKind::ProjectedEqualPointPointDistances3 {
                ...
                self.push_residual(
                    ...
                    rows,
                    constraint,
                    format!("{} projected equal point distances", constraint.name),
            ...
            SketchConstraintKind::ProjectedPointDistanceDifference3 {
                ...
                // This is the point-pair counterpart of projected line-length
                // difference. The square-root-free equality row proves the
                // metric relation, and the branch inequality makes the
                // intended longer point-pair explicit instead of implicit in
                // a proposal solver.
                let Some((parts, difference)) = self.projected_point_distance_difference_exprs(
                    workplane,
                    (longer_a, longer_b),
                ...
                self.push_residual(
                    ...
                    rows,
                    constraint,
                    format!("{} projected point squared difference", constraint.name),
                    sum_minus_difference.clone() * sum_minus_difference
                        - Expr::int(4) * parts.first_squared.clone() * parts.second_squared.clone(),
                    SketchResidualStrategy::SquaredProjectedPointDistanceDifference,
                );
                self.push_residual_with_kind(
                    ...
                    rows,
                    constraint,
                    format!("{} projected point longer branch", constraint.name),
                    parts.first_squared - parts.second_squared - difference_squared,
                    SketchResidualStrategy::SquaredProjectedPointDistanceDifference,
                    ConstraintKind::GreaterOrEqual,
                    Real::one(),
            ...
            SketchConstraintKind::ProjectedEqualPointDistancePointLineDistance3 {
                ...
                // This is the point-pair analogue of projected line-length to
                ...
                self.push_residual(
                    ...
                    parts.point_squared * parts.distance_denominator - parts.distance_numerator,
                    SketchResidualStrategy::SquaredProjectedPointDistancePointLineDistance,
                );
            }
            SketchConstraintKind::ProjectedEqualPointLineDistances3 {
            ...
            SketchConstraintKind::ProjectedCubicLineTangent3 {
                ...
                // This is the projected counterpart of retained cubic/line
                // tangency: exact workplane-frame replay, exact Bernstein
                // cubic point/derivative replay, and an explicit tangent
                // orientation branch. The candidate path never normalizes the
                // projected line tangent or cubic derivative.
                let Some(parts) = self.projected_cubic_line_tangent_exprs(
                    workplane,
                    cubic,
                    parameter,
                ...
                self.push_residual(
                    ...
                    rows,
                    constraint,
                    format!("{} projected cubic tangent support", constraint.name),
            ...
            SketchConstraintKind::ProjectedCubicCurveCubicCurveTangent3 {
                ...
                // This is the workplane-projected analogue of retained
                // cubic/cubic G1 tangency. Both 3D control nets are projected
                // first, then exact Bernstein point/derivative replay supplies
                // the proof rows.
                let Some(parts) = self.projected_cubic_curve_cubic_curve_tangent_exprs(
                    workplane,
                    first,
                ...
                for (axis, residual) in parts.tangent.point_incidence.into_iter().enumerate() {
                    self.push_residual(
                        problem,
                        rows,
                        constraint,
                        format!(
                            "{} projected 3D cubic/cubic point coordinate {axis}",
                            constraint.name
                        ),
                        residual,
                ...
                self.push_residual(
                    ...
                    constraint,
                    format!(
                        "{} projected 3D cubic/cubic tangent support",
                        constraint.name
                    ),
                    parts.tangent.tangent_cross,
                    SketchResidualStrategy::ProjectedCubicCurveCubicCurveTangent,
                ...
                self.push_residual_with_kind(
                    ...
                    constraint,
                    format!(
                        "{} projected 3D cubic/cubic tangent orientation",
                        constraint.name
                    ),
                    parts.tangent.tangent_dot,
                    SketchResidualStrategy::ProjectedCubicCurveCubicCurveTangent,
            ...
            SketchConstraintKind::ProjectedCubicCurveCubicCurveC2Continuity3 {
                ...
                // Projected parametric C2 keeps the same strict retained
                // parameterization as ordinary cubic C2, but proves every
                // coordinate after exact workplane projection.
                let Some(parts) = self.projected_cubic_curve_cubic_curve_c2_exprs(
                    workplane,
                    first,
                    first_parameter,
                ...
                for (axis, residual) in parts.c2.point.into_iter().enumerate() {
                    self.push_residual(
                        problem,
                        rows,
                        constraint,
                        format!(
                            "{} projected 3D c2 point coordinate {axis}",
                            constraint.name
                        ),
                        residual,
                        SketchResidualStrategy::ProjectedCubicCurveCubicCurveC2Continuity,
                ...
                for (axis, residual) in parts.c2.first_derivative.into_iter().enumerate() {
                    self.push_residual(
                        problem,
                        rows,
                        constraint,
                        format!(
                            "{} projected 3D c2 first derivative {axis}",
                            constraint.name
                        ),
                        residual,
                        SketchResidualStrategy::ProjectedCubicCurveCubicCurveC2Continuity,
                ...
                for (axis, residual) in parts.c2.second_derivative.into_iter().enumerate() {
                    self.push_residual(
                        problem,
                        rows,
                        constraint,
                        format!(
                            "{} projected 3D c2 second derivative {axis}",
                            constraint.name
                        ),
                        residual,
                        SketchResidualStrategy::ProjectedCubicCurveCubicCurveC2Continuity,
            ...
            SketchConstraintKind::ProjectedArcCubicCurveTangent3 {
                ...
                // Projected arc/cubic tangency uses the same retained
                // endpoint/radius/perpendicular/orientation rows as ordinary
                // arc/cubic tangency, after projecting the native 3D cubic
                // control net through the certified workplane frame.
                let Some(parts) = self.projected_arc_cubic_curve_tangent_exprs(
                    workplane,
                    arc,
                    arc_endpoint,
                ...
                self.push_residual(
                    ...
                    parts.tangent.arc_endpoint_radius,
                    SketchResidualStrategy::ProjectedArcCubicCurveTangent,
                );
                for (axis, residual) in parts.tangent.endpoint_incidence.into_iter().enumerate() {
                    self.push_residual(
                        problem,
            ...
            SketchConstraintKind::ProjectedPointOnCubicCurve3 {
                ...
                // This is retained 3D cubic incidence through a workplane
                // projection: the point and every cubic control point are
                // projected into U/V before exact Bernstein replay. Projection
                // and curve evaluation both remain proof rows rather than
                // proposal-side coordinate baking.
                let Some(parts) = self.projected_point_on_cubic_curve3_exprs(
                    workplane, point, cubic, parameter, constraint, rows,
                ) else {
                    return;
                ...
                for (axis, residual) in parts.incidence.into_iter().enumerate() {
                    self.push_residual(
                        ...
                        rows,
                        constraint,
                        format!("{} projected 3D cubic coordinate {axis}", constraint.name),
                        residual,
    ...
    fn projected_equal_point_point_distances_exprs(
        &self,
        workplane: SketchEntityHandle,
        pairs: [(SketchEntityHandle, SketchEntityHandle); 2],
        constraint: &SketchConstraint,
        rows: &mut Vec<SketchGeneratedRow>,
    ) -> Option<ProjectedEqualPointPointDistancesExprs> {
        let quaternion = self.workplane_quaternion(workplane, constraint, rows)?;
        let first_squared =
            self.projected_point_pair_distance_squared(pairs[0], &quaternion, constraint, rows)?;
        let second_squared =
            self.projected_point_pair_distance_squared(pairs[1], &quaternion, constraint, rows)?;
        Some(ProjectedEqualPointPointDistancesExprs {
            quaternion,
            first_squared,
            second_squared,
        })
    ...
    fn projected_point_radius_equality_exprs(
        &self,
        workplane: SketchEntityHandle,
        a: SketchEntityHandle,
        b: SketchEntityHandle,
        ...
        constraint: &SketchConstraint,
        rows: &mut Vec<SketchGeneratedRow>,
    ) -> Option<ProjectedPointRadiusEqualityExprs> {
        let quaternion = self.workplane_quaternion(workplane, constraint, rows)?;
        let point_squared =
            self.projected_point_pair_distance_squared((a, b), &quaternion, constraint, rows)?;
        let radius = self.circle_or_arc_radius_expr(curve, constraint, rows)?;
        Some(ProjectedPointRadiusEqualityExprs {
            quaternion,
            point_squared,
            radius,
        })
    ...
    fn projected_point_pair_distance_squared(
        &self,
        pair: (SketchEntityHandle, SketchEntityHandle),
        quaternion: &[Expr; 4],
        constraint: &SketchConstraint,
        rows: &mut Vec<SketchGeneratedRow>,
    ) -> Option<Expr> {
        let a = self.point3_coordinates(pair.0, constraint, rows)?;
        let b = self.point3_coordinates(pair.1, constraint, rows)?;
        let delta = [
            a.exprs[0].clone() - b.exprs[0].clone(),
            a.exprs[1].clone() - b.exprs[1].clone(),
            a.exprs[2].clone() - b.exprs[2].clone(),
        ];
        Some(projected_distance_squared(&delta, quaternion))
    }
    ...
    fn projected_point_distance_difference_exprs(
        &self,
        workplane: SketchEntityHandle,
        longer: (SketchEntityHandle, SketchEntityHandle),
        shorter: (SketchEntityHandle, SketchEntityHandle),
        ...
        constraint: &SketchConstraint,
        rows: &mut Vec<SketchGeneratedRow>,
    ) -> Option<(ProjectedEqualPointPointDistancesExprs, Expr)> {
        let quaternion = self.workplane_quaternion(workplane, constraint, rows)?;
        let first_squared =
            self.projected_point_pair_distance_squared(longer, &quaternion, constraint, rows)?;
        let second_squared =
            self.projected_point_pair_distance_squared(shorter, &quaternion, constraint, rows)?;
        let difference = self.distance_expr(difference, constraint, rows)?;
        Some((
            ProjectedEqualPointPointDistancesExprs {
                quaternion,
                first_squared,
                second_squared,
            },
    ...
    fn projected_equal_point_distance_point_line_distance_exprs(
        &self,
        workplane: SketchEntityHandle,
        point_pair: (SketchEntityHandle, SketchEntityHandle),
        point: SketchEntityHandle,
        line: SketchEntityHandle,
        constraint: &SketchConstraint,
        rows: &mut Vec<SketchGeneratedRow>,
    ) -> Option<ProjectedPointDistancePointLineDistanceExprs> {
        let quaternion = self.workplane_quaternion(workplane, constraint, rows)?;
        let point_squared =
            self.projected_point_pair_distance_squared(point_pair, &quaternion, constraint, rows)?;
        let (distance_numerator, distance_denominator) = self
            .projected_point_line_distance_squared_parts(
                point,
                line,
        ...
        Some(ProjectedPointDistancePointLineDistanceExprs {
            quaternion,
            point_squared,
            distance_numerator,
            distance_denominator,
    ...
    #[allow(clippy::too_many_arguments)]
    fn projected_arc_cubic_curve_tangent_exprs(
        &self,
        workplane: SketchEntityHandle,
        arc: SketchEntityHandle,
        arc_endpoint: SketchArcEndpoint,
        ...
        constraint: &SketchConstraint,
        rows: &mut Vec<SketchGeneratedRow>,
    ) -> Option<ProjectedArcCubicCurveTangentExprs> {
        let (origin, quaternion) =
            self.workplane_origin_and_quaternion(workplane, constraint, rows)?;
        let arc = self.arc2_endpoint_parts(arc, arc_endpoint, constraint, rows)?;
        ...
        Some(ProjectedArcCubicCurveTangentExprs {
            quaternion,
            tangent: arc_cubic_tangent_exprs(
                &coordinate_exprs2(&arc.center),
                &coordinate_exprs2(&arc.endpoint),
                arc.radius,
                &cubic.point,
    ...
    fn projected_point_on_cubic_curve3_exprs(
        &self,
        workplane: SketchEntityHandle,
        point: SketchEntityHandle,
        cubic: SketchEntityHandle,
        ...
        let projected_point = projected_direction2(&point_delta, &quaternion);
        let curve = cubic_bezier_point2_exprs(&p0, &p1, &p2, &p3, parameter);
        Some(ProjectedCubicBezierIncidenceExprs {
            quaternion,
            incidence: [
                projected_point[0].clone() - curve[0].clone(),
    ...
    fn projected_point3_coordinates(
        &self,
        point: SketchEntityHandle,
    ...
    #[allow(clippy::too_many_arguments)]
    fn projected_cubic_line_tangent_exprs(
        &self,
        workplane: SketchEntityHandle,
        cubic: SketchEntityHandle,
        parameter: SketchParameterHandle,
    ...
    #[allow(clippy::too_many_arguments)]
    fn projected_cubic_curve_cubic_curve_tangent_exprs(
        &self,
        workplane: SketchEntityHandle,
        first: SketchEntityHandle,
        first_parameter: SketchParameterHandle,
        ...
        constraint: &SketchConstraint,
        rows: &mut Vec<SketchGeneratedRow>,
    ) -> Option<ProjectedCubicCubicTangentExprs> {
        let (origin, quaternion) =
            self.workplane_origin_and_quaternion(workplane, constraint, rows)?;
        ...
        Some(ProjectedCubicCubicTangentExprs {
            quaternion,
            tangent: CubicCubicTangentExprs {
                point_incidence: [
    ...
    #[allow(clippy::too_many_arguments)]
    fn projected_cubic_curve_cubic_curve_c2_exprs(
        &self,
        workplane: SketchEntityHandle,
        first: SketchEntityHandle,
        first_parameter: SketchParameterHandle,
        ...
        constraint: &SketchConstraint,
        rows: &mut Vec<SketchGeneratedRow>,
    ) -> Option<ProjectedCubicCubicC2Exprs> {
        let (origin, quaternion) =
            self.workplane_origin_and_quaternion(workplane, constraint, rows)?;
        let first = self.projected_cubic3_point_tangent_exprs(
            first,
        ...
        Some(ProjectedCubicCubicC2Exprs {
            quaternion,
...
#[derive(Clone, Debug)]
struct ProjectedEqualPointPointDistancesExprs {
    quaternion: [Expr; 4],
    first_squared: Expr,
    second_squared: Expr,
}
...
#[derive(Clone, Debug)]
struct ProjectedPointRadiusEqualityExprs {
    quaternion: [Expr; 4],
    point_squared: Expr,
    radius: Expr,
}
...
#[derive(Clone, Debug)]
struct ProjectedCubicLineTangentExprs {
    quaternion: [Expr; 4],
    tangent: CubicLineTangentExprs,
}

#[derive(Clone, Debug)]
struct ProjectedCubicCubicTangentExprs {
    quaternion: [Expr; 4],
    tangent: CubicCubicTangentExprs,
}
...
#[derive(Clone, Debug)]
struct ProjectedPointDistancePointLineDistanceExprs {
    quaternion: [Expr; 4],
    point_squared: Expr,
    distance_numerator: Expr,
    distance_denominator: Expr,
