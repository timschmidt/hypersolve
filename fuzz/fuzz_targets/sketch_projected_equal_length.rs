...
fuzz_target!(|data: &[u8]| {
    ...
    let b0 = sketch.add_point3d("b0", Real::from(bx), Real::from(by), Real::from(bz));
    let b1 = sketch.add_point3d(
        "b1",
        Real::from(bx + dx),
        Real::from(by + dy),
        Real::from(bz - normal_offset),
    );
    let a = sketch.add_line_segment3("a", a0, a1);
    let b = sketch.add_line_segment3("b", b0, b1);
    let handle =
        sketch.add_projected_equal_length_lines3("projected equal length", workplane, a, b);
