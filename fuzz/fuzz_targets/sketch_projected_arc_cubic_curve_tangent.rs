...
fuzz_target!(|data: &[u8]| {
    ...
    let p1 = sketch.add_point3d(
        "p1",
        Real::from(p0_x),
        Real::from(oy + cy + tangent_sign * tangent),
        Real::from(oz + z1),
    );
    let p2 = sketch.add_point3d(
        "p2",
        Real::from(p0_x),
        Real::from(oy + cy + 2 * tangent_sign * tangent),
        Real::from(oz + z2),
    );
    let p3 = sketch.add_point3d(
        "p3",
        Real::from(p0_x),
        Real::from(oy + cy + 3 * tangent_sign * tangent),
        Real::from(oz + z3),
    );
    ...
    match mode {
        0 | 1 | 6 => assert!(certification.all_satisfied()),
        2 | 4 | 5 => assert!(!certification.all_satisfied()),
        _ => unreachable!(),
    }
    if mode == 2 {
        assert!(matches!(
