...
fuzz_target!(|data: &[u8]| {
    ...
    let qz1 = value(data, 22);
    let qz2 = value(data, 24);
    let qz3 = value(data, 26);
    let offset = i64::from(data[28] % 7) + 1;
    ...
    let q2 = sketch.add_point3d(
        "q2",
        Real::from(q0_x + 2 * dx),
        Real::from(oy + ay + 5 * dy),
        Real::from(oz + qz2),
    );
    let q3 = sketch.add_point3d(
        "q3",
        Real::from(q0_x + 3 * dx),
        Real::from(oy + ay + 6 * dy),
        Real::from(oz + qz3),
    );
    ...
    let handle = sketch.add_projected_cubic_curve_cubic_curve_g2_continuity3(
        "projected cubic curve/cubic curve g2",
        workplane,
        first,
        first_parameter,
        second,
