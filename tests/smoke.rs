...
#[test]
fn sketch_projected_cubic_curve_line_tangent_replays_projected_control_net_and_branch_rows() {
    let mut sketch = SketchSolveProblem::new();
    let origin = sketch.add_point3d("origin", real(1), real(2), real(3));
    let normal = sketch.add_normal3d("identity normal", real(1), real(0), real(0), real(0));
    let workplane = sketch.add_workplane("workplane", origin, normal);
    let p0 = sketch.add_point3d("p0", real(1), real(2), real(3));
    let p1 = sketch.add_point3d("p1", real(3), real(2), real(7));
    let p2 = sketch.add_point3d("p2", real(5), real(2), real(-5));
    let p3 = sketch.add_point3d("p3", real(7), real(2), real(11));
    let cubic = sketch.add_cubic3("cubic3", p0, p1, p2, p3);
    let parameter = sketch.add_parameter(
        "t",
    ...
    let reversed_end = sketch.add_point3d("reversed end", real(-2), real(2), real(8));
    let reversed = sketch.add_line_segment3("reversed", line_start, reversed_end);
    let wrong_cubic = sketch.add_cubic2("wrong cubic2", p0, p1, p2, p3);
    let valid = sketch_tangency_builders::projected_cubic_curve_line_tangent3(
        &mut sketch,
        "projected 3D cubic line tangent",
        workplane,
        cubic,
        parameter,
        line,
    ...
    sketch.add_projected_cubic_curve_line_tangent3(
        "reversed projected 3D cubic line tangent",
        workplane,
        cubic,
        parameter,
        reversed,
    ...
    let wrong = sketch.add_projected_cubic_curve_line_tangent3(
        "wrong projected 3D cubic line tangent kind",
        workplane,
        wrong_cubic,
        parameter,
        line,
    ...
    assert_eq!(
        forms.forms[1].kind,
        SketchResidualFormKind::ProjectedCubicCurveLineTangentEndpointIncidencePolynomial
    );
    assert_eq!(
        forms.forms[3].kind,
    ...
    assert!(forms.forms.iter().all(|form| {
        form.role == SketchResidualFormRole::ExactProof
            && (form.strategy == Some(SketchResidualStrategy::ProjectedCubicCurveLineTangent)
                || form.strategy == Some(SketchResidualStrategy::WorkplaneUnitQuaternion))
    }));
    for form in &forms.forms[..4] {
        assert_eq!(
...
#[test]
fn sketch_projected_cubic_curve_cubic_curve_c2_replays_projected_second_derivative_rows() {
    let mut sketch = SketchSolveProblem::new();
    let origin = sketch.add_point3d("origin", real(1), real(2), real(3));
    let normal = sketch.add_normal3d("identity normal", real(1), real(0), real(0), real(0));
    let workplane = sketch.add_workplane("workplane", origin, normal);
    ...
    let first = sketch.add_cubic3("first cubic3", a0, a1, a2, a3);
    let b0 = sketch.add_point3d("b0", real(4), real(2), real(-9));
    let b1 = sketch.add_point3d("b1", real(5), real(2), real(13));
    let b2 = sketch.add_point3d("b2", real(6), real(2), real(-15));
    let b3 = sketch.add_point3d("b3", real(7), real(2), real(17));
    let second = sketch.add_cubic3("second cubic3", b0, b1, b2, b3);
    let v0 = sketch.add_point3d("v0", real(4), real(2), real(19));
    let v1 = sketch.add_point3d("v1", real(5), real(2), real(-21));
    let v2 = sketch.add_point3d("v2", real(6), real(3), real(23));
    let v3 = sketch.add_point3d("v3", real(7), real(2), real(-25));
    let second_derivative_miss = sketch.add_cubic3("second derivative miss", v0, v1, v2, v3);
    let wrong_second = sketch.add_cubic2("wrong cubic2", b0, b1, b2, b3);
    let first_t = sketch.add_parameter("first t", real(1));
    let second_t = sketch.add_parameter("second t", real(0));
    let valid = sketch_tangency_builders::projected_cubic_curve_cubic_curve_c2_continuity3(
        &mut sketch,
    ...
    sketch.add_projected_cubic_curve_cubic_curve_c2_continuity3(
        "projected 3D cubic/cubic second derivative miss",
        workplane,
        first,
        first_t,
        second_derivative_miss,
        second_t,
    );
    let wrong = sketch.add_projected_cubic_curve_cubic_curve_c2_continuity3(
        "wrong projected 3D cubic/cubic c2 kind",
        workplane,
        first,
        first_t,
        wrong_second,
    ...
    assert!(lowered.rows[..14].iter().all(|row| {
    ...
    assert_eq!(
        forms.forms[1].kind,
        SketchResidualFormKind::ProjectedCubicCurveCubicCurveC2PointPolynomial
    );
    assert_eq!(
        forms.forms[3].kind,
        SketchResidualFormKind::ProjectedCubicCurveCubicCurveC2FirstDerivativePolynomial
    );
    assert_eq!(
        forms.forms[5].kind,
    ...
    assert!(forms.forms.iter().all(|form| {
        form.role == SketchResidualFormRole::ExactProof
            && (form.strategy
                == Some(SketchResidualStrategy::ProjectedCubicCurveCubicCurveC2Continuity)
                || form.strategy == Some(SketchResidualStrategy::WorkplaneUnitQuaternion))
    }));
    for form in &forms.forms {
        assert_eq!(
...
#[test]
fn sketch_projected_cubic_curve_cubic_curve_g2_replays_projected_curvature_rows() {
    let mut sketch = SketchSolveProblem::new();
    let origin = sketch.add_point3d("origin", real(1), real(2), real(3));
    let normal = sketch.add_normal3d("identity normal", real(1), real(0), real(0), real(0));
    let workplane = sketch.add_workplane("workplane", origin, normal);
    let a0 = sketch.add_point3d("a0", real(1), real(2), real(3));
    let a1 = sketch.add_point3d("a1", real(2), real(2), real(7));
    let a2 = sketch.add_point3d("a2", real(3), real(3), real(-5));
    let a3 = sketch.add_point3d("a3", real(4), real(5), real(11));
    let first = sketch.add_cubic3("first cubic3", a0, a1, a2, a3);
    let b0 = sketch.add_point3d("b0", real(4), real(5), real(-9));
    let b1 = sketch.add_point3d("b1", real(5), real(7), real(13));
    let b2 = sketch.add_point3d("b2", real(6), real(10), real(-15));
    let b3 = sketch.add_point3d("b3", real(7), real(14), real(17));
    let second = sketch.add_cubic3("second cubic3", b0, b1, b2, b3);
    let v0 = sketch.add_point3d("v0", real(4), real(5), real(19));
    let v1 = sketch.add_point3d("v1", real(5), real(7), real(-21));
    let v2 = sketch.add_point3d("v2", real(6), real(11), real(23));
    let v3 = sketch.add_point3d("v3", real(7), real(14), real(-25));
    let curvature_miss = sketch.add_cubic3("curvature miss", v0, v1, v2, v3);
    let wrong_second = sketch.add_cubic2("wrong cubic2", b0, b1, b2, b3);
    let first_t = sketch.add_parameter("first t", real(1));
    let second_t = sketch.add_parameter("second t", real(0));
    let valid = sketch_tangency_builders::projected_cubic_curve_cubic_curve_g2_continuity3(
        &mut sketch,
    ...
    sketch.add_projected_cubic_curve_cubic_curve_g2_continuity3(
        "projected 3D cubic/cubic curvature miss",
        workplane,
        first,
        first_t,
        curvature_miss,
    ...
    let wrong = sketch.add_projected_cubic_curve_cubic_curve_g2_continuity3(
        "wrong projected 3D cubic/cubic g2 kind",
        workplane,
        first,
        first_t,
        wrong_second,
    ...
    assert!(certification.rows[..7].iter().all(|row| {
        matches!(
            row.status,
            CertifiedCandidateStatus::CertifiedZero { .. }
                | CertifiedCandidateStatus::CertifiedSatisfiedInequality { .. }
    ...
    assert_eq!(
        forms.forms[1].kind,
        SketchResidualFormKind::ProjectedCubicCurveCubicCurveG2PointIncidencePolynomial
    );
    assert_eq!(
        forms.forms[3].kind,
        SketchResidualFormKind::ProjectedCubicCurveCubicCurveG2TangentCrossProductPredicate
    );
    assert_eq!(
        forms.forms[5].kind,
    ...
    assert_eq!(
        forms.forms[6].kind,
        SketchResidualFormKind::ProjectedCubicCurveCubicCurveG2CurvatureSignPredicate
    );
    for form in [
        &forms.forms[0],
        &forms.forms[1],
...
#[test]
fn sketch_projected_arc_cubic_curve_tangent_replays_projected_control_net_and_branch_rows() {
    let mut sketch = SketchSolveProblem::new();
    let origin = sketch.add_point3d("origin", real(1), real(2), real(3));
    let normal = sketch.add_normal3d("identity normal", real(1), real(0), real(0), real(0));
    let workplane = sketch.add_workplane("workplane", origin, normal);
    ...
    let radius = sketch.add_distance("radius", real(5));
    let arc = sketch.add_arc_of_circle2("arc", center, start, end, radius);
    let p0 = sketch.add_point3d("p0", real(6), real(2), real(3));
    let p1 = sketch.add_point3d("p1", real(6), real(4), real(7));
    let p2 = sketch.add_point3d("p2", real(6), real(6), real(-5));
    let p3 = sketch.add_point3d("p3", real(6), real(8), real(11));
    let cubic = sketch.add_cubic3("cubic3", p0, p1, p2, p3);
    let bad_p1 = sketch.add_point3d("bad p1", real(6), real(0), real(13));
    let bad_p2 = sketch.add_point3d("bad p2", real(6), real(-2), real(17));
    let bad_p3 = sketch.add_point3d("bad p3", real(6), real(-4), real(19));
    let bad_cubic = sketch.add_cubic3("bad cubic3", p0, bad_p1, bad_p2, bad_p3);
    let wrong_cubic = sketch.add_cubic2("wrong cubic2", start, p1, p2, p3);
    let t0 = sketch.add_parameter("t0", real(0));
    let valid = sketch_tangency_builders::projected_arc_cubic_curve_tangent3(
        &mut sketch,
    ...
    sketch.add_projected_arc_cubic_curve_tangent3(
        "bad projected arc cubic tangent",
        workplane,
        arc,
        SketchArcEndpoint::Start,
        bad_cubic,
    ...
    let wrong = sketch.add_projected_arc_cubic_curve_tangent3(
        "wrong projected arc cubic tangent kind",
        workplane,
        arc,
        SketchArcEndpoint::Start,
        wrong_cubic,
    ...
    assert_eq!(lowered.rows.len(), 13);
    assert!(lowered.rows[..12].iter().all(|row| {
    ...
    assert_eq!(
        forms.forms[1].kind,
        SketchResidualFormKind::ProjectedArcCubicCurveTangentEndpointRadiusPolynomial
    );
    assert_eq!(
        forms.forms[2].kind,
        SketchResidualFormKind::ProjectedArcCubicCurveTangentEndpointIncidencePolynomial
    );
    assert_eq!(
        forms.forms[4].kind,
        SketchResidualFormKind::ProjectedArcCubicCurveTangentRadiusPerpendicularPolynomial
    );
    assert_eq!(
        forms.forms[5].kind,
...
#[test]
fn sketch_projected_arc_cubic_curve_second_order_contact_replays_projected_circle_jet_rows() {
    let mut sketch = SketchSolveProblem::new();
    let origin = sketch.add_point3d("origin", real(1), real(2), real(3));
    let normal = sketch.add_normal3d("normal", real(1), real(0), real(0), real(0));
    let workplane = sketch.add_workplane("workplane", origin, normal);
    ...
    let arc = sketch.add_arc_of_circle2("arc", center, start, end, radius);
    let p0 = sketch.add_point3d("p0", real(6), real(2), real(3));
    let p1 = sketch.add_point3d("p1", real(6), real(12), real(7));
    let p2 = sketch.add_point3d("p2", real(-24), real(22), real(-5));
    let p3 = sketch.add_point3d("p3", real(-54), real(32), real(11));
    let cubic = sketch.add_cubic3("cubic3", p0, p1, p2, p3);
    let bad_p2 = sketch.add_point3d("bad p2", real(-23), real(22), real(17));
    let bad_cubic = sketch.add_cubic3("bad cubic3", p0, p1, bad_p2, p3);
    let wrong_cubic = sketch.add_cubic2("wrong cubic2", start, p1, p2, p3);
    let t0 = sketch.add_parameter("t0", real(0));
    let valid = sketch_tangency_builders::projected_arc_cubic_curve_second_order_contact3(
        &mut sketch,
    ...
    sketch.add_projected_arc_cubic_curve_second_order_contact3(
        "bad projected arc cubic second order contact",
        workplane,
        arc,
        SketchArcEndpoint::Start,
        bad_cubic,
    ...
    let wrong = sketch.add_projected_arc_cubic_curve_second_order_contact3(
        "wrong projected arc cubic second order contact kind",
        workplane,
        arc,
        SketchArcEndpoint::Start,
        wrong_cubic,
    ...
    assert_eq!(
        forms.forms[1].kind,
        SketchResidualFormKind::ProjectedArcCubicCurveSecondOrderEndpointRadiusPolynomial
    );
    assert_eq!(
        forms.forms[2].kind,
        SketchResidualFormKind::ProjectedArcCubicCurveSecondOrderEndpointIncidencePolynomial
    );
    assert_eq!(
        forms.forms[4].kind,
        SketchResidualFormKind::ProjectedArcCubicCurveSecondOrderRadiusPerpendicularPolynomial
    );
    assert_eq!(
        forms.forms[5].kind,
...
#[test]
fn sketch_projected_point_radius_equality_lowers_exact_workplane_rows() {
    let mut sketch = SketchSolveProblem::new();
    let origin = sketch.add_point3d("origin", real(10), real(20), real(30));
    let normal = sketch.add_normal3d("normal", real(1), real(0), real(0), real(0));
    let workplane = sketch.add_workplane("workplane", origin, normal);
    let a = sketch.add_point3d("a", real(10), real(20), real(30));
    let b = sketch.add_point3d("b", real(13), real(24), real(99));
    let bad = sketch.add_point3d("bad", real(16), real(20), real(99));
    let center = sketch.add_point2d("center", real(2), real(3));
    let start = sketch.add_point2d("start", real(5), real(7));
    let end = sketch.add_point2d("end", real(-2), real(6));
    let radius = sketch.add_distance("radius", real(5));
    let arc = sketch.add_arc_of_circle2("arc", center, start, end, radius);

    let valid = sketch_distance_builders::projected_point_radius_equality3(
        &mut sketch,
        "projected point radius equality",
        workplane,
        a,
        b,
        arc,
    ...
    sketch.add_projected_point_radius_equality3(
        "bad projected point radius equality",
        workplane,
        a,
        bad,
        arc,
    );
    let wrong = sketch.add_projected_point_radius_equality3(
        "wrong projected point radius equality",
        workplane,
        a,
        b,
        radius,
...
#[test]
fn sketch_projected_line_radius_equality_lowers_exact_workplane_rows() {
    let mut sketch = SketchSolveProblem::new();
    let origin = sketch.add_point3d("origin", real(0), real(0), real(0));
    let normal = sketch.add_normal3d("normal", real(1), real(0), real(0), real(0));
    let workplane = sketch.add_workplane("workplane", origin, normal);
    let line0 = sketch.add_point3d("line0", real(2), real(3), real(17));
    let line1 = sketch.add_point3d("line1", real(5), real(7), real(29));
    let bad1 = sketch.add_point3d("bad1", real(8), real(3), real(29));
    let line = sketch.add_line_segment3("line", line0, line1);
    let bad_line = sketch.add_line_segment3("bad line", line0, bad1);
    let center = sketch.add_point2d("center", real(10), real(10));
    let start = sketch.add_point2d("start", real(13), real(14));
    let end = sketch.add_point2d("end", real(6), real(13));
    let radius = sketch.add_distance("radius", real(5));
    let arc = sketch.add_arc_of_circle2("arc", center, start, end, radius);

    let valid = sketch_distance_builders::projected_line_radius_equality3(
        &mut sketch,
        "projected line radius equality",
        workplane,
        line,
        arc,
    );
    sketch.add_projected_line_radius_equality3(
        "bad projected line radius equality",
        workplane,
        bad_line,
        arc,
    );
    let wrong = sketch.add_projected_line_radius_equality3(
        "wrong projected line radius equality",
        workplane,
        line,
        radius,
    );
...
#[test]
fn sketch_projected_point_distance_difference3_lowers_exact_workplane_rows() {
    let mut sketch = SketchSolveProblem::new();
    let origin = sketch.add_point3d("origin", real(0), real(0), real(0));
    let normal = sketch.add_normal3d("normal", real(1), real(0), real(0), real(0));
    let workplane = sketch.add_workplane("workplane", origin, normal);
    ...
    let bad1 = sketch.add_point3d("bad1", real(8), real(0), real(0));
    let difference = sketch.add_distance("difference", real(5));
    let valid = sketch_distance_builders::projected_point_distance_difference3(
        &mut sketch,
    ...
    sketch.add_projected_point_distance_difference3(
        "violated projected point difference",
        workplane,
        (bad0, bad1),
        (short0, short1),
        difference,
    );
    let wrong_line = sketch.add_line_segment3("wrong line", long0, long1);
    let wrong = sketch.add_projected_point_distance_difference3(
        "wrong projected point difference",
        workplane,
        (long0, wrong_line),
        (short0, short1),
        difference,
...
#[test]
fn sketch_projected_equal_point_distance_point_line_distance3_lowers_exact_workplane_rows() {
    let mut sketch = SketchSolveProblem::new();
    let origin = sketch.add_point3d("origin", real(0), real(0), real(0));
    let normal = sketch.add_normal3d("normal", real(1), real(0), real(0), real(0));
    let workplane = sketch.add_workplane("workplane", origin, normal);
    let a = sketch.add_point3d("a", real(0), real(20), real(-7));
    let b = sketch.add_point3d("b", real(0), real(25), real(13));
    let bad_b = sketch.add_point3d("bad b", real(0), real(24), real(13));
    let distance_point = sketch.add_point3d("distance point", real(0), real(5), real(99));
    let distance0 = sketch.add_point3d("distance0", real(-10), real(0), real(-3));
    let distance1 = sketch.add_point3d("distance1", real(10), real(0), real(21));
    let distance_line = sketch.add_line_segment3("distance line", distance0, distance1);
    let valid = sketch_distance_builders::projected_equal_point_distance_point_line_distance3(
        &mut sketch,
    ...
    sketch.add_projected_equal_point_distance_point_line_distance3(
        "violated projected point distance equals point-line distance",
        workplane,
        a,
        bad_b,
        distance_point,
    ...
    let wrong = sketch.add_projected_equal_point_distance_point_line_distance3(
        "wrong projected point distance equals point-line distance",
        workplane,
        a,
        distance_line,
        distance_point,
...
#[test]
fn simplify_preserves_zero_division_and_zero_power_domains() {
    let x = Expr::symbol(SymbolId(0), "x");
    assert!(matches!((Expr::zero() / x).simplify(), Expr::Div(_, _)));

    let zero_power = Expr::zero().powi(0).simplify();
    assert!(matches!(zero_power, Expr::PowI(_, 0)));
    assert!(
        zero_power
            .eval_real(&std::collections::HashMap::new())
            .is_err()
    );
    assert_eq!(Expr::int(2).powi(0).simplify(), Expr::int(1));
    assert!(matches!(
        (Expr::zero() * (Expr::int(1) / Expr::symbol(SymbolId(1), "y"))).simplify(),
        Expr::Mul(_, _)
    ));
}
...
#[test]
fn candidate_certification_refines_nonstructural_satisfied_inequality_signs() {
    let unresolved_negative = Real::new(Rational::fraction(103_993, 33_102).unwrap()) - Real::pi();
    assert_eq!(unresolved_negative.structural_facts().sign, None);

    let x = Expr::symbol(SymbolId(0), "x");
    let mut problem = Problem::default();
    problem.add_variable("x", unresolved_negative);
    problem.add_constraint(Constraint {
        name: "sqrt(2) <= 2".to_owned(),
        kind: ConstraintKind::LessOrEqual,
        residual: x,
        weight: Real::one(),
        active: true,
    ...
    assert!(matches!(
        prepared.evaluate_residuals(&context).unwrap()[0]
            .value
            .certified_sign_until(-128),
        hyperreal::CertifiedRealSign::Known {
...
#[test]
fn negative_residual_ball_certifies_inequality_satisfaction() {
    let x = Expr::symbol(SymbolId(0), "x");
    let mut problem = Problem::default();
    problem.add_variable("x", real(-10));
    problem.add_constraint(Constraint {
...
fn residual_ball_certification_preserves_approximate_policy_certainty() {
    ...
    assert!(matches!(
        ...
        CertifiedCandidateStatus::BallCertified {
            sign: hyperreal::RealSign::Zero,
            certainty: hyperlimit::Certainty::Approximate,
            satisfied: true,
        }
    ));
}
...
fn residual_ball_certification_rejects_negative_radius() {
    ...
    assert_eq!(report.domain_failure_rows, 1);
}
...
#[test]
fn affine_interval_candidate_refines_nonstructural_radius_sign() {
    let x = Expr::symbol(SymbolId(0), "x");
    let mut problem = Problem::default();
    problem.add_variable("x", real(0));
    problem.add_constraint(Constraint::equality("x", x));
    let unresolved_negative = Real::new(Rational::fraction(103_993, 33_102).unwrap()) - Real::pi();
    assert_eq!(unresolved_negative.structural_facts().sign, None);
    ...
    let error = certify_affine_interval_candidate(
        ...
        &[VariableBall {
            symbol: SymbolId(0),
            radius: unresolved_negative,
        }],
        hyperlimit::PredicatePolicy::STRICT,
    )
    .expect_err("a refined negative radius must be rejected");
...
#[test]
fn candidate_domain_certification_reports_zero_power_and_simplified_zero_division() {
    let x = Expr::symbol(SymbolId(0), "x");
    let mut problem = Problem::default();
    problem.add_variable("x", real(0));
    problem.add_constraint(Constraint::equality(
        "zero power invalid",
        x.clone().powi(0).simplify(),
    ));
    problem.add_constraint(Constraint::equality(
        "zero numerator still needs denominator",
        (Expr::zero() / x.clone()).simplify(),
    ));
    problem.add_constraint(Constraint::equality(
        "zero product still needs denominator",
        (Expr::zero() * (Expr::int(1) / x)).simplify(),
    ));
    ...
    assert!(analysis.affine_residuals()[0].is_none());
    assert!(analysis.univariate_quadratic_residuals()[0].is_none());
    assert!(analysis.quadratic_residuals()[0].is_none());
    assert!(
        analysis
            .evaluate_constraint_residual(0, &context_from_problem(&problem))
            .is_err()
    );
    assert!(report.checks.iter().any(|check| {
        check.kind == DomainCheckKind::ZeroPowerBaseNonZero
            && check.status == DomainCheckStatus::CertifiedInvalid
    }));
    assert!(report.checks.iter().any(|check| {
        check.kind == DomainCheckKind::DivisionDenominatorNonZero
