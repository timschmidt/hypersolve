...
profile_data="${target_dir}/hypersolve.profdata"
report_dir="${target_dir}/html"
text_report="${target_dir}/coverage.txt"
object_manifest="${target_dir}/test-objects.txt"
minimum_production_line_coverage="${HYPERSOLVE_MIN_PRODUCTION_LINE_COVERAGE:-85}"
...
export LLVM_PROFILE_FILE="${profile_dir}/hypersolve-%p-%m.profraw"
...
run_configuration() {
    ...
    # Isolated symbol namespaces allow LLVM to merge the inert and traced
    # dependency-dispatch bodies without profile-hash collisions.
    export RUSTFLAGS="${coverage_rustflags} -C metadata=hypersolve_coverage_${metadata}"
    cargo test --locked "${cargo_args[@]}" --lib --tests --no-run --message-format=json |
        record_test_objects
    cargo test --locked "${cargo_args[@]}" --lib --tests --quiet
}

run_configuration "no features" no_features --no-default-features
run_configuration "dispatch tracing" dispatch_trace --all-features

# Criterion test mode executes every fixture once. Include examples and the
# dispatch tracer, while suppressing generated Markdown writes during coverage.
echo "Coverage configuration: all-feature targets and benchmark fixtures"
export RUSTFLAGS="${coverage_rustflags} -C metadata=hypersolve_coverage_all_targets"
cargo test --locked --all-features --all-targets --no-run --message-format=json |
    record_test_objects
HYPERSOLVE_SKIP_BENCHMARK_REPORTS=1 \
...
ignore_regex='/([.]cargo/registry|[.]rustup|rustc|hyperreal|hyperlattice|hyperlimit|nalgebra|roots|target|tests|benches|examples|fuzz)/'
...
# LLVM combines production code with inline unit-test modules. Derive a
# physical executable-line view that stops only at an explicit test-gated
# `mod tests` boundary; isolated `#[cfg(test)]` imports/helpers remain harmless.
echo
